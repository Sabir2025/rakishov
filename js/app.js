...
function openMenu(){dialog.showModal();document.body.style.overflow='hidden';burger.setAttribute('aria-expanded','true');const r=burger.getBoundingClientRect();const cx=r.left+r.width/2;const cy=r.top+r.height/2;dialog.style.setProperty('--nav-x',cx+'px');dialog.style.setProperty('--nav-y',cy+'px');dialog.getBoundingClientRect();requestAnimationFrame(()=>dialog.classList.add('open'))}
